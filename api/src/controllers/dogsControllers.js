...
try {
    let dogsApi = (await axios(`https://api.thedogapi.com/v1/breeds?api_key=${YOUR_API_KEY}`)).data
    let dogsApiInfo = dogsApi.map( dog => {
        return {
            ...
            temperaments: dog.temperament?dog.temperament: 'No hay temperamentos asignados',
        }
    })
    return dogsApiInfo
} catch (error) {
