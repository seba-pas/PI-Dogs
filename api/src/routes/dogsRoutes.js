...
const { getAllDogs, getDbDogs, getApiDogs} = require('../controllers/dogsControllers');
...
router.get('/', async (req,res) => {        
  ...
  if (name) {
    ...
    search.length? res.status(200).send(search) : res.status(400).send(`No Breeds including ${name}`);
  } else {
...
router.get('/:id', async (req, res) => {
  ...
  try {
    let dogsFromDb = [];
    if (id >= 265) {
       dogsFromDb = await Dogs.findAll({ where: { id: id }, include: Temperaments });
    }
